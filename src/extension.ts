...
export function activate(context: vscode.ExtensionContext) {
  ...
  // Now provide the implementation of the command with  registerCommand
  // The commandId parameter must match the command field in package.json
  let disposableES6 = vscode.commands.registerCommand(
    'extension.generateGettersAndSettersES6',
    () => {
      // The code you place here will be executed every time your command is executed
...
`;
  for (let p in properties) {
    while (properties[p].startsWith(' ')) {
      properties[p] = properties[p].substr(1);
    }
    while (properties[p].startsWith('\t')) {
      properties[p] = properties[p].substr(1);
    }
    ...
    if (create) {
      let code = `
  public get${Attribute}(): ${type}${isOptional ? ' | undefined' : ''} {
    return this.${attribute};
  }

  public set${Attribute}(${attribute}${isOptional ? '?' : ''}: ${type}): void {
...
`;
  for (let p in properties) {
    ...
    public set ${attribute.replace('_', '')}(${attribute.replace(
        '_',
        '',
      )}: ${type}) {
